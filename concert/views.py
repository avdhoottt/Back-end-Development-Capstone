...
def signup(request):
    ...
    POST: Process signup form, create user if doesn't exist
    """
...
def songs(request):
    """
    Display songs from dummy data
    Will be connected to microservice later
    """
    songs = {
        "songs": [
            {
...
def photos(request):
    """
    Display photos from dummy data
    Will be connected to pictures microservice later
    Returns: Rendered photos template with dummy photo data
    """
    photos = [{
        "id": 1,
...
def concerts(request):
    ...
    Shows concert details and user's attendance status
    """
...
def concert_detail(request, id):
    """
    Display details of a specific concert
    Requires authentication
    """
    if request.user.is_authenticated:
...
def concert_attendee(request):
    """
    Handle concert attendance status updates
    Requires authentication
    """
    if request.user.is_authenticated:
        if request.method == "POST":
